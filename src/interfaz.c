...
void I_Update(IState *state, ListaComponentes *componentesID, ListaConexiones *conexionesID) {
    ...
    if (!state->dibujando) {
        ...
        if (LeftClick(btnPanel)) {
            AgregarComponentes(componentesID, panel_solar, mouse.x, mouse.y, 12, 0, 550, 0, 0, -1, -1);
        }
        ...
        for (int i = 0; i < conexionesID->cuenta; i++) {
            if ((conexionesID->conexiones[i].origenID == id_eliminado ||conexionesID->conexiones[i].destinoID == id_eliminado) && id_eliminado != -1) {
                ...
                for (int j = i; j < conexionesID->cuenta - 1; j++) {
                    conexionesID->conexiones[j] = conexionesID->conexiones[j + 1];
                }
                conexionesID->cuenta--;
                i--; 
            }
